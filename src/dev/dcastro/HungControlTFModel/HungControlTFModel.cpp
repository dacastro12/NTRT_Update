...
//tibia and fibia (Cross Beams)
    ...
    //knee joint left
    nodePositions.push_back(btVector3(1.5*c.factor, (c.Knee_height+2)*c.factor, 0)); // 6 Was 1.5 for x
    //knee joint right
    nodePositions.push_back(btVector3( -1.5*c.factor, (c.Knee_height+2)*c.factor, 0)); // 7 Was -1.5 for x
    


//femur
    // knee joint front (patella)
    nodePositions.push_back(btVector3(0, (c.Knee_height+2)*c.factor, 2*c.factor)); // 8
    // knee joint left
    nodePositions.push_back(btVector3(1.25*c.factor, (c.Knee_height+2)*c.factor, -1.25*c.factor)); //9 Was 1.25 for x and -z
    // knee joint right
    nodePositions.push_back(btVector3(-1.25*c.factor, (c.Knee_height+2)*c.factor, -1.25*c.factor)); //10 Was 1.25 for -x and -z
    // knee joint origin
